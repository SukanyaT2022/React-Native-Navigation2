...
import BoxProtectionScreenComp, {
  ArrayItemType,
...
//   data:
// }
const wrapGroupArray: {id: string; title: string; data: ArrayItemType[]}[] = [
  {
    id: '1',
    title: 'data1',
    data: [
      {
        title: 'No deductible for vehicle loss or damage.',
        check: true,
        id: '1',
      },
    ...
    <View style={styles.mainView}>
      ...
      <View style={styles.wrapFlatlistStyle}>
        <FlatList
          ...
          renderItem={({item}) => {
            // disableVar make btn only can select one btn and disabl thr rest
            var disabledVar = selectedID !== null && selectedID !== item.id;
         
            return <BoxProtectionScreenComp
              checkTextDetailArray={item.data}
              priceProp="32.47"
              titleProp={item.title}
              recommendTagProp="Recommended"
              disabled = {disabledVar}
              // line 95 in harge of change color only when clicked
              selected={disabledVar ? false : selectedID === item.id}
